...
## Stronger ZS baseline selection

For the requested alternative, set `selection_mode` to `stronger_zs` and `ours_fraction` to `0.70`. Keep `preserve_t1_from` pointing to the accepted T1 export. This expands T2/T3 to 288/82 eligible candidates while keeping the same model stages and cumulative classes. Every baseline must retain visible foreground and nonzero overlap; Ours must exceed every baseline by at least 0.10. Rank first by the weakest Dice among ZS-Sequential, ZS-EWC and ZS-GPM, then minimum pairwise disagreement, Ours Dice and lower index. This prioritizes baseline quality over visual differences. It does not guarantee that every individual ZS score improves versus a previously selected slice.

`selection_description` may record this rule verbatim in the private report. Default mode remains the previous diversity ranking with Ours at 90% of its eligible maximum. Both modes are intentionally selected qualitative comparisons, not average performance estimates. The self-check covers the stronger-ZS ranking as well.
